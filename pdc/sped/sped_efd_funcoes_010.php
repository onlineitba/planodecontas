...
if($box_inventario=='on'){
    ...
    $motivo_inventario=trim($_POST[motivo_inventario]);
    if ($motivo_inventario=='') {
   echo "Informe um motivo para inventário!";
	echo _myfuncoes_voltar_pagina();
        exit;
...
function sped_efd_registro_0000() {
             global $info_segmento,$fp,$lanperiodo1,$lanperiodo2,$pcind_perfil,$cod_ver,$info_cnpj_segmento;
                      ...
                      if($ind_ativ<>'0'){
                         $ind_ativ='1'; // SPED aceita apenas 0 ou 1.
                      }                                                                                                                                                                                                                  
                      $linha='|0000|'.$cod_ver.'|'.$cod_finalidade.'|'.$dt_ini.'|'.$dt_fin.'|'.$xnome.'|'.$xcnpj.'|'.$xcpf.'|'.$info_segmento[uf].'|'.$info_segmento[ie].'|'.$cmun.'|'.$im.'|'.$info_segmento[suframa].'|'.$pcind_perfil.'|'.$ind_ativ.'|';
...
function sped_efd_registro_0100() {
                ...
                $sel_cnpj = mysql_query("SELECT a.*,b.razao,b.tel,b.fax,b.cep,b.endereco,b.num,b.compl,b.bairro,b.cod_mun,b.email FROM $TCONTABILISTA as a,$TCNPJCPF as b WHERE a.cpf=b.cnpj",$CONTCONTABILISTA);
...
function sped_efd_registro_0150() {
	...
	global $qtd_lin_0,$info_segmento,$fp,$TLANCAMENTOS,$TCNPJCPF,$lanperiodo1,$lanperiodo2,$livro_reg,$tabela_1600;

		$tabela_1600=sped_tabelas_temp_1600(); 
	       ...
	       $sql_participantes="select a.cnpjcpf,b.razao from $livro_reg as a,$TCNPJCPF as b where a.cnpjcpf=b.cnpj group by a.cnpjcpf UNION select a.cnpjcpfoperadora as cnpjcpf,b.razao from $tabela_1600 as a,$TCNPJCPF as b where a.cnpjcpfoperadora=b.cnpj group by a.cnpjcpfoperadora order by razao";
...
global $arq_inventario,$box_inventario,$info_cnpj; 
...
// O Código do Item deverá ser preenchido com as informações utilizadas na última ocorrência do período.
 ...
 $sel_itens=_myfunc_sql_receitas_despesas_itens('','') ;
         ...
         $modelos_inclusos="|01|1B|2D|04|06|07|08|8B|09|10|11|21|22|26|27|28|29|57|";
...
$sql_190="SELECT b.*,a.ucom FROM $livro_reg as a,$TUNIDADE_FATOR_CONVERSAO as b where b.unidad_conv=a.ucom and $filtro_modelos group by a.ucom order by a.ucom ASC";
     	$selunidade = mysql_query("$sql_190",$CONTITEM_FLUXO);
           ...
           if($box_inventario=="on"){
                  ...
                  $sql_0200="select ucom,cprod,tipo_lancamento,modelo from(select ucom,cprod,tipo_lancamento,modelo from $livro_reg as a where $filtro_modelos UNION (select unidade as ucom,cprod,'PRODUTOS' as tipo_lancamento,'01' as modelo from $arq_inventario)) as Tab0200 group by cprod";
...
function sped_efd_registro_0400() {
                ...
                $filtromodelo="and POSITION( modelo IN ':03:06:08:21:22:28:29:57')=0";


                $xsqlx="SELECT dono,cod_sit,modelo,if(movimento='RECEITAS','0','1') as ind_emit,movimento,codnat,valor,cnpjcpf from $TLANCAMENTOS where cnpjcpfseg='$info_cnpj_segmento' and ((data >= $perdtos1) and (data <= $perdtos2)) and lan_impostos<>'S' and movimento<>'TRANSFERE' $filtromodelo group by codnat,modelo,cod_sit order by codnat";
				...
				$movimento=$pdono[movimento];
                                $ind_emit=$pdono[ind_emit];  // 0- propria,1-terceiro
 				IF (($info_cnpj_segmento==$pdono[cnpjcpf]) and $movimento=='DESPESAS') {
   ...
   $modelos_inclusos="|01|1B|04|55|";  //
    ...
    while ($v=mysql_fetch_assoc($sql_lancamentos)) {
		  ...
		  $cod_part=$v[cnpjcpf];
		    ...
		    $cod_mod=$v['modelo'];
			  ...
			  $cod_sit=(empty($v[cod_sit]))  ? '00' : $v[cod_sit];  // se for receitas ,emissão propria
				...
				// Não apresentar chave qdo documento for modelo 01
				if($cod_mod=='01'){ 
					$chv_nfe='';
				}
		...
		$ind_frt='9';
		 ...
		 if ($ind_emit=='1' or $info_nfdocumentos[modelo]=='01') { 
...
function sped_efd_registro_c420(){
	  ...
	  _matriz_linha($linha1);
			...
			$filtroaliq="and (cst='$npbx' or cst='010' or cst='070' or cst='051' or cst='050') and serial_imp='$serialimpressora'"; // para filhos
                   sped_efd_registro_c425($dtreducaoz,$filtroaliq);
                }

             }
 $filtroaliq="and (cst='$npbx' or cst='041') and serial_imp='$serialimpressora'"; // para filtro no c425
...
//                         $xsql_d="SELECT cst,picms,cfop,sum(vprod-vdesc) as vvprod,sum(vbc) as vvbc,sum(vicms) as vvicms,sum(vbcst) as vvbcst,sum(vicmsst) as vvicmsst,sum(vipi) as vvipi,sum(vdesc) as vvdesc,modelo FROM tabela_c425_provisorio where data='$dtreducaoz' and modelo='2D' group by cst,cfop,picms order by id";
		         $xsql_d="SELECT cst,picms,cfop,sum(vprod-vdesc) as vvprod,sum(vbc) as vvbc,sum(vicms) as vvicms,sum(vbcst) as vvbcst,sum(vicmsst) as vvicmsst,sum(vipi) as vvipi,sum(vdesc) as vvdesc,modelo FROM tabela_c425_provisorio where data='$dtreducaoz' and modelo='2D' and serial_imp='$serialimpressora' group by cst,cfop,picms order by id";
...
function sped_efd_registro_D001(){
		...
		$filtro_lancamentos=" and POSITION(modelo IN '|07|08|8B|09|10|11|21|22|26|27|57|') > 0";
        $sql_lancamentos= _myfunc_sql_receitas_despesas_documentos('',"$filtro_lancamentos",'','','');
...
function sped_efd_registro_D190($xdono) {
                   global $livro_reg,$info_segmento,$fp,$REG_BLC,$TITEM_FLUXO,$CONTITEM_FLUXO,$TPRODUTOS,$TSERVICOS,$info_cnpj_segmento,$lanperiodo1,$lanperiodo2,$TLANCAMENTOS;

 	$xsql_d=_myfunc_sql_receitas_despesas_itens('',$xdono);
 
 //      $xsql_y="SELECT *,sum(vprod) as vvprod,sum(vbc) as vvbc,sum(vicms) as vvicms,sum(vfrete) as vvfrete,sum(vseg) as vvseg,sum(voutro) as vvoutro,sum(vbcst) as vvbcst,sum(vicmsst) as vvicmsst,sum(vipi) as vvipi,sum(vdesc) as vvdesc FROM $livro_reg  where dono='$xdono' group by cst,cfop,picms order by dono,cst,cfop,picms";
  //    $xsql_d=mysql_query("$xsql_y",$CONTITEM_FLUXO);  
...
// REGISTRO E116: OBRIGAÇÕES DO ICMS A RECOLHER OPERAÇÕES PRÓPRIAS.
 function sped_efd_registro_E116() {
              ...
              */
              ...
              $linha1='|E116|'. $cod_or.'|'.$vl_icms_recolher.'|'.$dt_vcto.'|'.$codigo_receita_obrigacao.'|'.$num_proc.'|'.$ind_proc.'|'.$proc.'|';
...
function sped_efd_registro_H001() {
             global $info_segmento,$fp,$box_inventario;
                 $qtde_linha_bloco_h++;
                 $linha ="|H001|1|";
                 if($box_inventario=="on"){
			                 $linha ="|H001|0|";
...
function sped_efd_registro_H005() {
                        global $sdata_inventario,$arq_inventario,$info_segmento,$fp,$TITEM_FLUXO,$CONTITEM_FLUXO,$info_cnpj_segmento,$perdtos1,$perdtos2,$qtde_linha_bloco_h,$TPRODUTOS,$lanperiodo2,$REG_BLC;

 			$xsql_h120="SELECT * from $arq_inventario where sqtotal>0   order by cprod ";
 
   	                $sql_lancamentos= mysql_query("$xsql_h120");
                        $row =mysql_num_rows($sql_lancamentos);
 
                        $vl_inv=0;
                        if ($row>0) {
                           while ( $v = mysql_fetch_assoc($sql_lancamentos) ) {

				  $vl_unit=$v[custo_medio];
				  if ($vl_unit<=0) {
				     $vl_unit=$v[custo_documento];
				  }
				  $vl_item=$vl_unit*$v[sqtotal];
                                 $vl_inv=$vl_inv+ $vl_item;

                           }
                        }
                        $vr_inv=number_format($vl_inv, 2, ",", "")  ;
                               if ($vl_inv==0) {
                                   $vr_inv='0,00';
                               }
                        $dt_fin=_myfunc_ddmmaaaa(_myfunc_stod($lanperiodo2));
   $dt_inventario=_myfunc_ddmmaaaa(_myfunc_stod($sdata_inventario)); 
$motivo_inventario=trim($_POST[motivo_inventario]);
                        $linha = "|H005|$dt_inventario|$vr_inv|$motivo_inventario|";
			 _matriz_linha($linha);

                        RETURN ;  // RETORNA VALOR DO INVENTÁRIO, CASO 0 NAO GERA O H010
...
function sped_efd_registro_H010() {
                        global $arq_inventario,$info_segmento,$fp,$TITEM_FLUXO,$CONTITEM_FLUXO,$info_cnpj_segmento,$perdtos1,$perdtos2,$qtde_linha_bloco_h,$TPRODUTOS,$lanperiodo2,$REG_BLC;
                        $xsql_h120="SELECT * from $arq_inventario  order by cprod ";
   	                $sql_lancamentos= mysql_query("$xsql_h120");
			$row =mysql_num_rows($sql_lancamentos);
			...
			$tot_registro_bloco_H010=0;
					...
					$qtd=number_format($qtd, 2, ",", "");
					  ...
					  $vl_item=$vl_unit*$v[sqtotal];
		                      $ind_prop='0';
		                      $cod_part='';
		                      $txt_compl=trim($v[descricao]);
		                      $cod_cta='';
		                      $vl_unit=number_format($vl_unit, 2, ",", "")  ;
			               ...
			               $linha= "|H010|".$cod_item."|".$unid."|".$qtd."|".$vl_unit."|".$vl_item."|".$ind_prop."|".$cod_part."|".$txt_compl."|".$cod_cta."|".$vl_item_ir."|";
...
// Adic. 03/02/2016 - Dados entram em vigor a partir de 2017, por momento deve ser gerado apenas com abertura e fechamento
//BLOCO K: CONTROLE DA PRODUÇÃO E DO ESTOQUE
// REGISTRO K001: ABERTURA DO BLOCO K

sped_efd_registro_K001();

sped_efd_registro_K990();


// REGISTRO K001: ABERTURA DO BLOCO K
function sped_efd_registro_K001() {
             global $info_segmento,$fp,$qtde_linha_bloco_k,$REG_BLC;
                 $qtde_linha_bloco_k++;
                 $linha="|K001|1|";
				 _matriz_linha($linha);
                 return;
...
function sped_efd_registro_K990() {
        global $info_segmento,$fp;
            $qtde_linha_bloco=_myfunc_qtde_registro_matriz_linha_sped('K') + 1;
            $linha='|K990|'.$qtde_linha_bloco.'|';
		    _matriz_linha($linha);
			echo "<br> BLOCO K";
			flush();
...
if ($cod_ver>='006') {
	sped_efd_registro_1010();
		...
		function sped_efd_registro_1010() {
			     global $info_segmento,$fp,$tabela_1600;
				...
				$result=mysql_query("select * from $tabela_1600");
...
function sped_efd_registro_1600(){  
         global $info_segmento,$fp,$qtde_linha_bloco,$qtde_linha_bloco_1,$REG_BLC,$lanperiodo1,$lanperiodo2;
         global $info_cnpj_segmento,$TLANCAMENTOS,$TDOCFINANCEIRO,$tot_registro_bloco_1600,$tabela_1600;

          $sql=mysql_query("select cnpjcpfoperadora,sum(vdeb) as debito,sum(vcred) as credito from $tabela_1600 group by cnpjcpfoperadora");

          while ($v=mysql_fetch_assoc($sql)) {

                 $reg='1600';

                 $cod_part=$v['cnpjcpfoperadora'];
                 $tot_credito=number_format($v['credito'], 2, ",", "")  ;
                 $tot_debito=number_format($v['debito'], 2, ",", "")  ;

                 $linha='|'.$reg.'|'.$cod_part.'|'.$tot_credito.'|'.$tot_debito.'|';
                 _matriz_linha($linha);
...
// Verificar vendas com cartões 
function sped_tabelas_temp_1600(){
         global $info_segmento,$info_cnpj_segmento;
         global $TITEM_FLUXO,$CONTITEM_FLUXO,$TLANCAMENTOS,$TNFDOCUMENTOS,$TDOCFINANCEIRO,$perdtos1,$perdtos2,$lanperiodo1,$lanperiodo2;

         $docs_cartao='';
         $sql=mysql_query("SELECT tipodoc,cnpjcpfoperadora,tiporeferente FROM $TDOCFINANCEIRO where cnpjcpfoperadora<>'' group by tipodoc");

         while ($v=mysql_fetch_assoc($sql)) {
              $tipodoc=$v[tipodoc];
              $docs_cartao=$docs_cartao."|".$tipodoc;
              //echo $docs_cartao."<br>";
          }

  		 $filtro_lancamentos=" and POSITION(a.modelo IN '|01|1B|04|55|2D|') > 0 and POSITION(a.tipodocfinanceiro IN '$docs_cartao')>0";
         //$sql_lancamentos= _myfunc_sql_receitas_despesas_documentos('',"$filtro_lancamentos",'','','');
         $filtro_modelos_e_df=" WHERE ((a.data >= $perdtos1) and (a.data <= $perdtos2)) $filtro_lancamentos";

         $sql_lancamentos="select if(b.tiporeferente='CD',sum(a.valor),0.00) as vdeb,if(b.tiporeferente='CC',sum(a.valor),0.00) as vcred,sum(a.valor) as svalor,b.cnpjcpfoperadora,b.tiporeferente from $TLANCAMENTOS as a,$TDOCFINANCEIRO as b $filtro_modelos_e_df and a.tipodocfinanceiro=b.tipodoc group by b.tiporeferente,b.cnpjcpfoperadora order by b.cnpjcpfoperadora";
         $lancamentos_pdf='lancamentos_1600_'.$info_cnpj_segmento;

         $sql_error='';
         $sql="drop table IF EXISTS $lancamentos_pdf";  // apaga anterior
         if ( mysql_query($sql) or die (mysql_error()) ) {
         }

         $sql_c1600="create table $lancamentos_pdf as $sql_lancamentos";
         if ( mysql_query($sql_c1600) or die (mysql_error()) ) {
            }
 ...
 // VERIFICAÇÃO DE DOCs EM ABERTO QUE CONTENHAM NÚMERO DE DOCUMENTO - TABELAS TMP
function sped_efd_registro_abertos() {

	global $qtd_lin_0,$info_segmento,$fp,$TLANCAMENTOS_TMP,$TNFDOCUMENTOS_TMP,$TCNPJCPF,$lanperiodo1,$lanperiodo2;

	       $modelos_inclusos="|01|1B|04|06|07|08|8B|09|10|11|21|22|26|27|28|29|55|57|";
	       $filtro_modelos=" and POSITION(a.modelo IN '$modelos_inclusos')>0";
 
	       $sql_docs_tmp="select a.numero,a.xml_importado_entrada,a.modelo,a.data,a.dono from $TNFDOCUMENTOS_TMP as a where (a.data >= $lanperiodo1  and a.data <= $lanperiodo2) $filtro_modelos group by a.dono order by a.numero";
		   
 	       $sel_docs_tmp=mysql_query("$sql_docs_tmp");
           while ( $vp = mysql_fetch_assoc($sel_docs_tmp)) {	     

		     $xdono=$vp['dono']; 
			 $xnum=$vp['numero'];

		     $sql_abertos="select a.dono,a.documento,a.modelo,a.cod_sit,a.cnpjcpf,a.codnat,a.data,b.razao from $TLANCAMENTOS_TMP as a,$TCNPJCPF as b where a.dono='$xdono' and a.cnpjcpf=b.cnpj group by a.dono order by a.documento";
 	         $sel_abertos=mysql_query("$sql_abertos");
             while ( $vab = mysql_fetch_assoc($sel_abertos)) {	     
                
				$dono=$vab['dono'];
		        $cod_part=trim($vab[cnpjcpf]);
		        $partipantes=_myfun_dados_cnpjcpf("$cod_part");
		        $nome=$partipantes['razao'];
                $data=_myfunc_stod($vab['data']);

		        $documento=$vab['documento'];
		        $modelo=$vab['modelo'];
		        $valor=$vab['valor'];
		        $natureza=$vab['codnat'];

		        $linha='|'.$dono.'|'.$xnum.'|'.$data.'|'.$cod_part.'|'.$nome.'|'.$documento.'|'.$modelo.'|'.$valor.'|'.$natureza.'|';
